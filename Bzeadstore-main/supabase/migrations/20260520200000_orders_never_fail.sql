-- =============================================================================
-- ORDERS MUST NEVER FAIL — relax the FX hard-fail introduced by
-- 20260520180000_audit_hardening.sql.
--
-- Rule: if the buyer paid (Stripe captured the money) the order MUST be
-- persisted. We still refuse to silently corrupt money math, so when an FX
-- rate is missing we:
--   (a) fall back to 1.0 (or the INR rate where applicable),
--   (b) flag the order via additive `orders.fx_warning text` so finance can
--       reconcile post-hoc.
--
-- Functions touched (CREATE OR REPLACE only; signatures unchanged):
--   - lock_order_fx_snapshot     (BEFORE INSERT trigger)
--   - refresh_order_seller_subtotal(p_order_id uuid)
--   - create_order_secure(...)   (signature already updated by 20260520180000)
-- =============================================================================

BEGIN;

-- ----------------------------------------------------------------------------
-- 0. Additive audit-flag column. Nullable. Existing rows untouched.
-- ----------------------------------------------------------------------------
ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS fx_warning text;

COMMENT ON COLUMN public.orders.fx_warning IS
  'Non-null when create_order_secure or FX triggers had to fall back to a default rate because countries.exchange_rate was missing. Finance must reconcile these orders.';

-- ----------------------------------------------------------------------------
-- 1. lock_order_fx_snapshot — never raise; fall back + flag.
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Safe fallback: never block order persistence. Flag instead.
  IF v_buyer_rate IS NULL OR v_buyer_rate <= 0 THEN
    v_warn := concat_ws('; ', v_warn, 'lock_fx: buyer rate missing for ' || v_buyer_ccy);
    v_buyer_rate := 1;
  END IF;
  IF v_seller_rate IS NULL OR v_seller_rate <= 0 THEN
    v_warn := concat_ws('; ', v_warn, 'lock_fx: seller rate missing for ' || v_seller_ccy);
    v_seller_rate := 1;
  END IF;
  ...
  IF v_warn IS NOT NULL THEN
    NEW.fx_warning := concat_ws(' | ', NEW.fx_warning, v_warn);
  END IF;
...
-- ----------------------------------------------------------------------------
-- 2. refresh_order_seller_subtotal — never raise; fall back + flag.
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_inr_rate    IS NULL OR v_inr_rate    <= 0 THEN v_warn := concat_ws('; ', v_warn, 'refresh: INR rate missing');                     v_inr_rate    := 1; END IF;
  IF v_buyer_rate  IS NULL OR v_buyer_rate  <= 0 THEN v_warn := concat_ws('; ', v_warn, 'refresh: buyer rate missing for '  || v_buyer_ccy);  v_buyer_rate  := 1; END IF;
  IF v_seller_rate IS NULL OR v_seller_rate <= 0 THEN v_warn := concat_ws('; ', v_warn, 'refresh: seller rate missing for ' || v_seller_ccy); v_seller_rate := 1; END IF;
  ...
  SET seller_items_subtotal     = round(v_seller_items, 2),
      ...
      fx_warning = case
        when v_warn is null then o.fx_warning
        else concat_ws(' | ', o.fx_warning, v_warn)
      end
...
-- ----------------------------------------------------------------------------
-- 3. create_order_secure — never raise on missing FX; fall back + flag.
--    All other behavior (idempotency, stock decrement, commission, items
--    insert) identical to 20260520180000.
-- ----------------------------------------------------------------------------
...
begin
  ...
  -- ---- FX RATE: derive server-side from countries; soft-fail with flag ----
  ...
  if v_inr_rate is null or v_inr_rate <= 0 then
    v_fx_warning := concat_ws('; ', v_fx_warning, 'create_order: INR rate missing');
    v_inr_rate := 1;
  end if;
  if v_buyer_rate is null or v_buyer_rate <= 0 then
    v_fx_warning := concat_ws('; ', v_fx_warning, 'create_order: buyer rate missing for ' || v_buyer_ccy);
    v_buyer_rate := v_inr_rate;  -- 1:1 vs INR rather than absolute 1, so math stays in same scale
  end if;
