-- Fix create_order_secure to use variant prices correctly:
-- Single variant product → always use product selling price (p.price)
-- Multiple variant product → use selected variant price (pv.price)
-- Also drop the old overload without p_fx_rate parameter.

DROP FUNCTION IF EXISTS public.create_order_secure(uuid, jsonb, jsonb, jsonb, text, text, text, text, text, text, text, numeric, numeric, numeric);
    ...
    ROUND(
      CASE
        WHEN (SELECT COUNT(*) FROM product_variants WHERE product_id = p.id) > 1
          THEN COALESCE(pv.price, p.price)
        ELSE p.price
      END * v_fx * (item->>'quantity')::INT * 0.91, 2
    )
  FROM jsonb_array_elements(p_items) item
  JOIN products p ON p.id = (item->>'product_id')::UUID
  LEFT JOIN product_variants pv
