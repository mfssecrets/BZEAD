-- =============================================================================
-- AUDIT HARDENING — close the 4 gaps surfaced in the 2026-05-20 audit
--   (1) Realtime push: add orders + order_items to supabase_realtime
--   (2) Persist Stripe processing fee on orders
--   (3) Backend-only FX: create_order_secure ignores client p_fx_rate and
--       derives the rate from countries; raises if unavailable
--   (4) Remove silent INR fallback in lock_order_fx_snapshot and
--       refresh_order_seller_subtotal
-- All other behavior, columns, RLS, and call sites remain unchanged.
-- =============================================================================

BEGIN;

-- ----------------------------------------------------------------------------
-- 1. REALTIME — publish orders + order_items so seller / admin UIs can subscribe
-- ----------------------------------------------------------------------------
DO $$
BEGIN
  IF NOT EXISTS (
...
-- ----------------------------------------------------------------------------
-- 2. STRIPE FEE column on orders (additive; populated by stripe-webhook)
-- ----------------------------------------------------------------------------
ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS stripe_fee numeric(18,2);

COMMENT ON COLUMN public.orders.stripe_fee IS
  'Stripe processing fee in BUYER currency, captured from balance_transaction.fee at webhook time. Locked.';

-- ----------------------------------------------------------------------------
-- 3. lock_order_fx_snapshot — hard-fail when FX is missing
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_buyer_rate IS NULL OR v_buyer_rate <= 0 THEN
    RAISE EXCEPTION 'FX rate unavailable for buyer currency %', v_buyer_ccy
      USING ERRCODE = 'P0001';
  END IF;
  IF v_seller_rate IS NULL OR v_seller_rate <= 0 THEN
    RAISE EXCEPTION 'FX rate unavailable for seller currency %', v_seller_ccy
      USING ERRCODE = 'P0001';
  END IF;
...
-- ----------------------------------------------------------------------------
-- 4. refresh_order_seller_subtotal — hard-fail when FX is missing
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_inr_rate    IS NULL OR v_inr_rate    <= 0 THEN RAISE EXCEPTION 'FX rate unavailable for INR'                                                     USING ERRCODE = 'P0001'; END IF;
  IF v_buyer_rate  IS NULL OR v_buyer_rate  <= 0 THEN RAISE EXCEPTION 'FX rate unavailable for buyer currency %',  v_buyer_ccy  USING ERRCODE = 'P0001'; END IF;
  IF v_seller_rate IS NULL OR v_seller_rate <= 0 THEN RAISE EXCEPTION 'FX rate unavailable for seller currency %', v_seller_ccy USING ERRCODE = 'P0001'; END IF;
...
-- ----------------------------------------------------------------------------
-- 5. create_order_secure — derive FX server-side from countries;
--    raise loudly when unavailable. Ignore client-supplied p_fx_rate
--    (kept in signature only for backward call-site compatibility).
-- ----------------------------------------------------------------------------
...
begin
  ...
  -- ---- FX RATE: derive server-side from countries; hard-fail if missing ----
  ...
  if v_inr_rate is null or v_inr_rate <= 0 then
    raise exception 'FX rate unavailable for INR' using errcode = 'P0001';
  end if;
  if v_buyer_rate is null or v_buyer_rate <= 0 then
    raise exception 'FX rate unavailable for buyer currency %', v_buyer_ccy using errcode = 'P0001';
  end if;
