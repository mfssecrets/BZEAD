-- =============================================================================
-- RESTORE SERVER-SIDE FX DERIVATION IN create_order_secure
--
-- ★ CANONICAL — SINGLE SOURCE OF TRUTH for create_order_secure. ★
-- Any older migration that defines create_order_secure carries a SUPERSEDED
-- warning and must NOT be re-run on its own. This file is the definition that
-- matches the live database.
--
-- Regression: the client (Checkout.tsx) stopped passing p_fx_rate on
-- 2026-05-20 (commit d871fdb) and relies on the server deriving FX from
-- countries.exchange_rate. Migration 20260521120000_trust_countries_table
-- added that derivation, but the live function was reverted out-of-band to
-- `v_fx := greatest(coalesce(p_fx_rate, 1), 0.000001)`. With the client
-- omitting p_fx_rate this made v_fx = 1, so create_order_secure stored the
-- INR-magnitude product price (get_public_product_prices_with_overrides
-- returns base_price*(1+markup%) in INR, no FX) labelled with the buyer's
-- foreign currency. Only non-INR orders were affected; INR orders keep
-- v_fx = 1 and are unchanged.
--
-- This migration re-applies the trust_countries definition verbatim:
--   v_fx := round(v_buyer_rate / v_inr_rate, 8)   -- INR -> buyer currency
-- Body is otherwise identical to the live function.
-- =============================================================================
...
GRANT EXECUTE ON FUNCTION public.create_order_secure(uuid, jsonb, jsonb, jsonb, text, text, text, text, text, text, text, numeric, numeric, numeric, numeric, text, text, text, text, text, timestamptz, integer, text) TO authenticated;
